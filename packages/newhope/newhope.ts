...
export type SeedExpanderConstructor = new (seedBytes: Uint8Array) => SeedExpander;

const Q = 12289; // prime that defines the field.
const N = 1024;  // Number of coefficients in polynomials.

// Byte length of an encoded polynomial.
// The encoding uses 14 bits per coefficient.
const ENCODED_POLY_LEN = (N * 14) / 8;

// Byte length of public seed.
export const PUBLIC_SEED_LENGTH = 32;

// Byte length of secret seed.
export const SECRET_SEED_LENGTH = 32;

// Byte length of the offering (first) message of the key exchange.
export const OFFER_MESSAGE_LENGTH = ENCODED_POLY_LEN + PUBLIC_SEED_LENGTH;

// Byte length of the accepting (second) message of the key exchange.
export const ACCEPT_MESSAGE_LENGTH = ENCODED_POLY_LEN + 256;

// Byte length of shared key.
export const SHARED_KEY_LENGTH = 32;

// Byte length of saved state.
export const SAVED_STATE_LENGTH = ENCODED_POLY_LEN;

// Polynomyal of N coefficients, N elements.
type Poly = Uint16Array;

// Reconciliation data, N bytes.
type ReconciliationData = Uint8Array;
...
// Expands a noise seed (key) into a pseudorandom byte stream.
// Used as a PRNG for noise generation.
class ChaChaNoiseSeedExpander implements SeedExpander {
    private _state = new Uint8Array(16); // 15-byte counter || 1-byte nonce.
    private _buf = new Uint8Array(64);
    private _bufpos = 64;

    constructor(private key: Uint8Array, nonceByte: number, noncePos = 0) {
        // Last 8 byte is a nonce to generate different keystreams
        // for different uses.
        this._state[8 + noncePos] = nonceByte;
    }

    stream(dst: Uint8Array): Uint8Array {
        for (let i = 0; i < dst.length; i++) {
            if (this._bufpos === 64) {
                // Generate next block.
                stream(this.key, this._state, this._buf, 8);
                this._bufpos = 0;
            }
    ...
    clean() {
        // this.key is not owned by us, so don't wipe it.
        wipe(this._buf);
        wipe(this._state);
    }
}
 ...
 */
export class NewHope implements KeyAgreement {
    ...
    /**
     * SecretSeed must be of SECRET_SEED_LENGTH.
     */
    constructor(
        secretSeed?: Uint8Array,
        private _prng?: RandomSource,
        private _expander: SeedExpanderConstructor = SHAKE128Expander,
        private _hash: new () => Hash = SHA3256) {
        if (secretSeed && secretSeed.length !== SECRET_SEED_LENGTH) {
    ...
    saveState(): Uint8Array {
        if (!this._sFreq) {
            throw new Error("NewHope: nothing to save");
        }
        return encodePoly(this._sFreq);
    }

    restoreState(serializedState: Uint8Array): this {
        if (serializedState.length !== this.savedStateLength) {
            throw new Error("NewHope: incorrect serialized state length");
        }
        this._sFreq = decodePoly(serializedState);
        return this;
    }
     ...
     *
     * Optional public seed must have PUBLIC_SEED_LENGTH random bytes.
     * If not provided, it's generated using randomBytes.
     */
    offer(seed = randomBytes(PUBLIC_SEED_LENGTH, this._prng)): Uint8Array {
        if (seed.length !== PUBLIC_SEED_LENGTH) {
            throw new Error("NewHope: incorrect seed length");
        }

        const aFreq = seedToPolynomial(seed, this._expander);

        const sFreq = forwardNTT(sampleNoiseFromSeed(this._secretSeed, 0));
        const eFreq = forwardNTT(sampleNoiseFromSeed(this._secretSeed, 1));

        const bFreq = new Uint16Array(N) as Poly;
        for (let i = 0; i < bFreq.length; i++) {
            bFreq[i] = (sFreq[i] * aFreq[i] + eFreq[i]) % Q;
        }

        const bFreqEnc = encodePoly(bFreq);
        const offerMsg = concat(bFreqEnc, seed);
    ...
    accept(offerMsg: Uint8Array): Uint8Array {
        if (this._sFreq) {
            throw new Error("NewHope: accept shouldn't be called by offering party");
        }
        if (offerMsg.length !== OFFER_MESSAGE_LENGTH) {
            throw new Error("NewHope: incorrect offer message length");
        }

        const bFreq = decodePoly(offerMsg.subarray(0, ENCODED_POLY_LEN));
        const seed = offerMsg.subarray(ENCODED_POLY_LEN);

        const aFreq = seedToPolynomial(seed, this._expander);

        const sPrimeFreq = forwardNTT(sampleNoiseFromSeed(this._secretSeed, 0));

        const ePrimeFreq = forwardNTT(sampleNoiseFromSeed(this._secretSeed, 1));

        const uFreq = new Uint16Array(N) as Poly;
        for (let i = 0; i < uFreq.length; i++) {
            uFreq[i] = (sPrimeFreq[i] * aFreq[i] + ePrimeFreq[i]) % Q;
        }

        const vFreq = new Uint16Array(N) as Poly;
        for (let i = 0; i < vFreq.length; i++) {
            vFreq[i] = (sPrimeFreq[i] * bFreq[i]) % Q;
        }

        const v = inverseNTT(vFreq);
        const ePrimePrime = sampleNoiseFromSeed(this._secretSeed, 2);
        for (let i = 0; i < v.length; i++) {
            v[i] = (v[i] + ePrimePrime[i]) % Q;
        }

        const rec = helprec(v, this._secretSeed, 3);

        const rawKey = reconcile(v, rec);
        this._sharedKey = hashSharedKey(rawKey, this._hash);

        const uFreqEnc = encodePoly(uFreq);
        const recEnc = encodeRec(rec);

        const acceptMsg = concat(uFreqEnc, recEnc);
    ...
    /**
     * Checks the accept message and generates shared key,
     * which can be retrieved using getSharedKey().
     */
    finish(acceptMsg: Uint8Array): this {
        if (!this._sFreq) {
            throw new Error("NewHope: no offer state");
        }

        if (acceptMsg.length !== ACCEPT_MESSAGE_LENGTH) {
            throw new Error("NewHope: incorrect accept message length");
        }

        const uFreq = decodePoly(acceptMsg.subarray(0, ENCODED_POLY_LEN));
        const rec = decodeRec(acceptMsg.subarray(ENCODED_POLY_LEN));

        for (let i = 0; i < uFreq.length; i++) {
            uFreq[i] = (uFreq[i] * this._sFreq[i]) % Q;
        }
        const u = inverseNTT(uFreq);
        const rawKey = reconcile(u, rec);

        this._sharedKey = hashSharedKey(rawKey, this._hash);

        // Remove used sFreq.
        wipe(this._sFreq);
        this._sFreq = undefined;
    ...
    getSharedKey(): Uint8Array {
        if (!this._sharedKey) {
            throw new Error("NewHope: no shared key established");
        }
        return new Uint8Array(this._sharedKey);
    }
}

function hashSharedKey(sharedKey: Uint8Array, hash: new () => Hash): Uint8Array {
    const h = new hash();
    h.update(sharedKey);
...
const OMEGAS_MONTGOMERY = new Uint16Array([
...
const OMEGAS_INV_MONTGOMERY = new Uint16Array([
...
const PSIS_BITREV_MONTGOMERY = new Uint16Array([
    4075, 6974, 7373, 7965, 3262, 5079, 522, 2169, 6364, 1018, 1041,
    8775, 2344, 11011, 5574, 1973, 4536, 1050, 6844, 3860, 3818, 6118,
    2683, 1190, 4789, 7822, 7540, 6752, 5456, 4449, 3789, 12142, 11973,
    382, 3988, 468, 6843, 5339, 6196, 3710, 11316, 1254, 5435, 10930,
...
const PSIS_INV_MONTGOMERY = new Uint16Array([
...
function bitrevVector(poly: Poly) {
    let tmp: number;
    for (let i = 0; i < N; i++) {
        const r = BITREV[i];
        if (i < r) {
...
function sampleNoise(expander: SeedExpander): Poly {
    const poly = new Uint16Array(N) as Poly;
    const buf = new Uint8Array(4);
    for (let i = 0; i < poly.length; i++) {
        expander.stream(buf);

        const t = readUint32LE(buf);

        let d = 0;
        for (let j = 0; j < 8; j++) {
            d += (t >>> j) & 0x01010101;
        }
        const a = ((d >>> 8) & 0xff) + (d & 0xff);
        const b = (d >>> 24) + ((d >>> 16) & 0xff);
...
function sampleNoiseFromSeed(noiseSeed: Uint8Array, nonceByte: number): Poly {
    const expander = new ChaChaNoiseSeedExpander(noiseSeed, nonceByte);
    const poly = sampleNoise(expander);
    expander.clean();
    return poly;
}

function randomPolynomial(expander: SeedExpander): Poly {
    const poly = new Uint16Array(N) as Poly;
    const buf = new Uint8Array(2);
    for (let i = 0; i < poly.length; i++) {
        while (true) {
            expander.stream(buf);
            const v = readUint16LE(buf) & 0x3fff; // XXX: specialized for Q = 12889
            if (v < Q) {
                poly[i] = v;
                break;
            }
...
// Converts 32-byte seed into pseudorandom polynomial.
function seedToPolynomial(seed: Uint8Array, expander: SeedExpanderConstructor) {
    const seedExpander = new expander(seed);
    const poly = randomPolynomial(seedExpander);
    seedExpander.clean();
    return poly;
}

function forwardNTT(poly: Poly): Poly {
    mulCoefficients(poly, PSIS_BITREV_MONTGOMERY);
    ntt(poly, OMEGAS_MONTGOMERY);
    return poly;
}

function inverseNTT(poly: Poly): Poly {
    bitrevVector(poly);
    ntt(poly, OMEGAS_INV_MONTGOMERY);
    mulCoefficients(poly, PSIS_INV_MONTGOMERY);
    return poly;
}

function mulCoefficients(poly: Poly, factors: Uint16Array) {
    for (let i = 0; i < N; i++) {
        poly[i] = montgomeryReduce(poly[i] * factors[i]);
    }
}

function ntt(a: Poly, omega: Uint16Array) {
    for (let i = 0; i < 10; i += 2) {
        ...
        for (let start = 0; start < distance; start++) {
            let jTwiddle = 0;
            for (let j = start; j < N - 1; j += 2 * distance) {
                const W = omega[jTwiddle++];
                const temp = a[j];
                a[j] = (temp + a[j + distance]); // Omit reduction (be lazy)
                a[j + distance] = montgomeryReduce(W * (temp + 3 * Q - a[j + distance]));
            }
        }
...
function barrettReduce(a: number) {
    let u = (a * 5) >>> 16;
    u *= Q;
    a -= u;
    return a >>> 0;
...
/*
// Small, but slow implementation:

// These values are used in the NTT calculation. See the paper for
// details about their origins.
const OMEGA = 49;
const INV_OMEGA = 1254;
const SQRT_OMEGA = 7;
const INV_SQRT_OMEGA = 8778;
const INV_N = 12277;

// Converts polynomial into the frequency domain.
function forwardNTT(poly: Poly): Poly {
    bitrevVector(poly); // this matches ref implementation but not required
    return ntt(poly, OMEGA, SQRT_OMEGA, 1, 1);
}

// Converts polynomial into the time domain.
function inverseNTT(poly: Poly): Poly {
    return ntt(poly, INV_OMEGA, 1, INV_SQRT_OMEGA, INV_N);
}

function ntt(poly: Poly, omega: number, preScaleBase: number,
    postScaleBase: number, postScale: number): Poly {
    const out = new Uint16Array(N) as Poly;
    let omegaToTheI = 1;
    ...
    for (let i = 0; i < out.length; i++) {
        ...
        for (let j = 0; j < poly.length; j++) {
            let t = (poly[j] * preScale) % Q;
            sum += (t * omegaToTheIJ) % Q;
            omegaToTheIJ = (omegaToTheIJ * omegaToTheI) % Q;
            preScale = (preScaleBase * preScale) % Q;
        }

        out[i] = (sum * postScale) % Q;

        omegaToTheI = (omegaToTheI * omega) % Q;
        postScale = (postScale * postScaleBase) % Q;
    }
    return out;
}
...
function encodeRec(rec: ReconciliationData): Uint8Array {
    const result = new Uint8Array(N / 4);
    for (let i = 0; i < result.length; i++) {
        result[i] = rec[4 * i] | (rec[4 * i + 1] << 2) | (rec[4 * i + 2] << 4) | (rec[4 * i + 3] << 6);
    }
    return result;
}

function decodeRec(message: Uint8Array): ReconciliationData {
    const rec = new Uint8Array(N) as ReconciliationData;
...
function encodePoly(poly: Poly): Uint8Array {
    const result = new Uint8Array(ENCODED_POLY_LEN);
    for (let i = 0; i < N / 4; i++) {
        const t0 = poly[4 * i + 0];
        const t1 = poly[4 * i + 1];
        const t2 = poly[4 * i + 2];
        const t3 = poly[4 * i + 3];
        result[7 * i + 0] = (t0);
        result[7 * i + 1] = ((t0 >>> 8) & 0xff) | ((t1 << 6) & 0xff);
        result[7 * i + 2] = (t1 >>> 2) & 0xff;
        result[7 * i + 3] = ((t1 >>> 10) & 0xff) | ((t2 << 4) & 0xff);
        result[7 * i + 4] = (t2 >>> 4) & 0xff;
        result[7 * i + 5] = ((t2 >>> 12) & 0xff) | ((t3 << 2) & 0xff);
        result[7 * i + 6] = (t3 >>> 6) & 0xff;
    }
    return result;
}


function decodePoly(encoded: Uint8Array): Poly {
    const result = new Uint16Array(N);
    for (let i = 0; i < N / 4; i++) {
        result[4 * i + 0] = (encoded[7 * i + 0]) | (encoded[7 * i + 1] & 0x3f) << 8;
        result[4 * i + 1] = (encoded[7 * i + 1]) >>> 6 | (encoded[7 * i + 2]) << 2 | (encoded[7 * i + 3] & 0x0f) << 10;
        result[4 * i + 2] = (encoded[7 * i + 3]) >>> 4 | (encoded[7 * i + 4]) << 4 | (encoded[7 * i + 5] & 0x03) << 12;
        result[4 * i + 3] = (encoded[7 * i + 5]) >>> 2 | (encoded[7 * i + 6]) << 6;
    }
    return result;
}
...
function f(x: number): [number, number, number] {
...
function helprec(v: Poly, secretSeed: Uint8Array, nonceByte: number): ReconciliationData {
    const expander = new ChaChaNoiseSeedExpander(secretSeed, nonceByte, 7);
    const randBits = new Uint8Array(N / (4 * 8));
    expander.stream(randBits);
    expander.clean();
    const ret = new Uint8Array(N) as ReconciliationData;
    for (let i = 0; i < N / 4; i++) {
        let rbit = (randBits[i >> 3] >> (i & 7)) & 1;

        const [a0, b0, k0] = f(8 * v[i] + 4 * rbit);
        const [a1, b1, k1] = f(8 * v[256 + i] + 4 * rbit);
        const [a2, b2, k2] = f(8 * (v[512 + i]) + 4 * rbit);
        const [a3, b3, k3] = f(8 * (v[768 + i]) + 4 * rbit);

        const k = (2 * Q - 1 - ((k0 + k1) + k2 + k3)) >> 31;
...
function ldDecode(xi0: number, xi1: number, xi2: number, xi3: number): number {
    let t = g(xi0);
    t += g(xi1);
    t += g(xi2);
    t += g(xi3);
...
function reconcile(v: Poly, rec: ReconciliationData): Uint8Array {
    let key = new Uint8Array(32);
    for (let i = 0; i < N / 4; i++) {
        const t0 = 16 * Q + 8 * (v[i]) - Q * (2 * (rec[i]) + (rec[i + 768]));
        const t1 = 16 * Q + 8 * (v[i + 256]) - Q * (2 * (rec[256 + i]) + (rec[i + 768]));
        const t2 = 16 * Q + 8 * (v[i + 512]) - Q * (2 * (rec[512 + i]) + (rec[i + 768]));
        const t3 = 16 * Q + 8 * (v[i + 768]) - Q * (rec[i + 768]);
