...
(async () => {
    ...
    report("scrypt 1024, 8, 1                        ",
        benchmark(() => deriveKey(password, salt, 1024, 8, 1, 32)));

    report("scrypt 32768, 8, 1                       ",
        benchmark(() => deriveKey(password, salt, 32768, 8, 1, 32)));

    report("scrypt 2**18, 8, 1                       ",
        benchmark(() => deriveKey(password, salt, 2 ** 18, 8, 1, 32)));

    let inst = new Scrypt(32768, 8, 1);
    report("scrypt 32768, 8, 1 (shared)              ",
        benchmark(() => inst.deriveKey(password, salt, 32)));
